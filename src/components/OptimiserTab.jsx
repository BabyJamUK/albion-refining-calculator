...
export default function OptimiserTab({ cascadeBase, prices, inventory, usageFee = 0, feeResolver, includeMarketFee = true, allowedCities = null, onOptimised }) {
...
return MATERIAL_TYPES.map(mat => {
  ...
  if (!hasAny) return null
   ...
   const base  = optimiseMaterial(mat, rawInv, prices, BASE_RETURN_RATE, effectiveFee, includeMarketFee, allowedCities)
   const bonus = optimiseMaterial(mat, rawInv, prices, CITY_RETURN_RATE, effectiveFee, includeMarketFee, allowedCities)

  return { mat, base, bonus, rawInv }
}).filter(Boolean)


}, [prices, cascadeBase, inventory, usageFee, feeResolver, includeMarketFee, allowedCities])
