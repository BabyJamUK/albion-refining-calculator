...
import { fetchPrices, buildItemIds } from '../services/marketApi'
...
// ─── Best city for selling refined ───────────────────────────────────────────
function bestSellCity(refinedApiId, prices) {
  const cityPrices = prices?.[refinedApiId]
  if (!cityPrices) return null
  return Object.entries(cityPrices).reduce(
    (best, [city, price]) => price > (best?.price ?? 0) ? { city, price } : best, null
  )
}

// ─── Profit for one tier row ──────────────────────────────────────────────────
function calcTierProfit(tierRow, prices, returnRate) {
  if (!prices) return []
  ...
  return ALL_CITIES.map(city => {
    ...
    const lowerRefinedPrice = lowerRefinedApiId
      ? (prices[lowerRefinedApiId]?.[city] ?? 0)
      : 0
    ...
    return { city, rawPrice, refinedPrice, rawValue, lowerCost, refinedValue, lowerRefinedPrice }
...
export default function InventoryInput() {
  ...
  const [cascadeBase, setCascadeBase] = useState(null)  // base return cascade results
  const [cascadeBonus,setCascadeBonus]= useState(null)  // city bonus cascade results
  ...
  function rerunCascades(inv, alloc) {
    ...
    const base  = MATERIAL_TYPES.map(mat => ({ ...mat, tiers: runCascade(mat, rawInv(mat), alloc[mat.id], BASE_RETURN_RATE)  })).filter(m => m.tiers.length)
    const bonus = MATERIAL_TYPES.map(mat => ({ ...mat, tiers: runCascade(mat, rawInv(mat), alloc[mat.id], CITY_RETURN_RATE) })).filter(m => m.tiers.length)
    setCascadeBase(base)
  ...
  const handleCalculate = async () => {
    ...
    try {
      const ids = buildItemIds(
        MATERIAL_TYPES.filter(m => base.find(r => r.id === m.id)),
        TIERS
      )
  ...
  // ── Derived: active results for display ─────────────────────────────────────
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-700">
                ...
                {TIERS.map(t => (
                  <th key={t} className="text-center py-2 px-2 text-gray-400 font-medium min-w-[130px]">
              ...
              {MATERIAL_TYPES.map(mat => (
                <tr key={mat.id} className="hover:bg-gray-800/50 transition-colors">
                  <td className="py-3 pr-4">
                    ...
                    <span className="text-xs text-gray-500">Bonus city: {mat.bonusCity}</span>
                  </td>
                  {TIERS.map(tier => (
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap gap-3 text-xs">
                ...
                <div className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 border border-gray-700">
                  ...
                  <span className="text-gray-300">Use slider to send refined <strong>up</strong> to next tier</span>
                </div>
              </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                              ...
                              <div className="bg-gray-800/60 rounded-lg p-3">
                                ...
                                {row.lowerRefinedName ? (
                                  ...
                                  <div className="text-xs text-gray-600">None (T3)</div>
                                )}
                              </div>
                              ...
                              <div className="bg-amber-900/20 border border-amber-700/30 rounded-lg p-4">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <span className="text-xs text-gray-400">
                                    Max available: <span className="text-amber-400 font-bold">{fmt(maxSendUp)}</span>
                                  </span>
                                </div>
                                ...
                                <div className="flex gap-4 mt-3 text-xs">
                                  <span className="text-gray-400">
                                    Sellable (base):
                                    <span className="text-blue-400 font-bold ml-1">{fmt(row.sellableRefined)}</span>
                                  </span>
                                  <span className="text-gray-400">
                                    Sellable (bonus):
                                    <span className="text-green-400 font-bold ml-1">{fmt(bonusRow?.sellableRefined)}</span>
                                  </span>
                                  <span className="text-gray-400">
                                    Sent up:
                                    <span className="text-amber-400 font-bold ml-1">{fmt(allocation[mat.id][row.tier])}</span>
                                  </span>
                                </div>
                              </div>
                            )}

                            {/* Final tier — no send up */}
                            {!hasNext && (
          ...
          {activeTab === 'profit' && (
            <div className="space-y-6">
              ...
              {prices && !loading && activeMats.map(mat => {
                ...
                return (
                  <div key={mat.id} className="bg-gray-900 rounded-xl border border-gray-700 overflow-hidden">
                    ...
                    {mat.tiers.map((row, idx) => {
                      const bonusRow   = bonusMat?.tiers[idx]
                      const baseProfit = calcTierProfit(row, prices, BASE_RETURN_RATE)
                      const bonusProfit= calcTierProfit(bonusRow ?? row, prices, CITY_RETURN_RATE)
                      ...
                      const bestBaseCity    = baseProfit.reduce((b, r) => r.refinedValue > (b?.refinedValue ?? 0) ? r : b, null)
                      const bestBonusCity   = bonusProfit.reduce((b, r) => r.refinedValue > (b?.refinedValue ?? 0) ? r : b, null)

                      const topValue = Math.max(
                        bestRawCity?.rawValue ?? 0,
                        bestBonusCity?.refinedValue ?? 0
                      )
                      const verdict =
                        (bestBonusCity?.refinedValue ?? 0) >= (bestRawCity?.rawValue ?? 0)
                        ...
                        <div key={row.tier} className="border-b border-gray-800 last:border-0">
                          ...
                          <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div className="bg-gray-800/40 rounded-lg p-3">
                              ...
                              {bestRawCity ? (
                                <>
                                  <div className="text-xs text-gray-400">{bestRawCity.city}</div>
                                  <div className="text-gray-200 font-mono font-bold">{silver(bestRawCity.rawValue)}</div>
                                </>
                              ) : <span className="text-gray-600 text-xs">No data</span>}
                            </div>
                            <div className="bg-blue-900/20 border border-blue-700/30 rounded-lg p-3">
                              <div className="text-xs text-blue-400 mb-1">Best refined (base rate)</div>
                              {bestBaseCity ? (
                                <>
                                  <div className="text-xs text-gray-400">{bestBaseCity.city}</div>
                                  <div className="text-blue-300 font-mono font-bold">{silver(bestBaseCity.refinedValue)}</div>
                                </>
                              ) : <span className="text-gray-600 text-xs">No data</span>}
                            </div>
                            <div className="bg-green-900/20 border border-green-700/30 rounded-lg p-3">
                              ...
                              {bestBonusCity ? (
                                <>
                                  <div className="text-xs text-gray-400">{bestBonusCity.city}</div>
                                  <div className="text-green-300 font-mono font-bold">{silver(bestBonusCity.refinedValue)}</div>
                                </>
                              ) : <span className="text-gray-600 text-xs">No data</span>}
                            </div>
                          </div>
                          ...
                          <div className="px-6 pb-4">
                            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold
                              ${verdict.color === 'green' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>
                              Recommendation: {verdict.label}
                              {topValue > 0 && <span className="font-mono ml-2">({silver(topValue)})</span>}
                            </div>
                          </div>
                            ...
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-b border-gray-800 text-gray-500 bg-gray-800/20">
                                  ...
                                  <th className="text-right px-4 py-2 text-blue-400">Refined total (base)</th>
                                  <th className="text-right px-4 py-2 text-green-400">Refined total (bonus)</th>
                              ...
                              <tbody className="divide-y divide-gray-800/40">
                                {ALL_CITIES.map(city => {
                                  ...
                                  if (!b && !bn) return null
                                  const best = Math.max(b?.rawValue ?? 0, bn?.refinedValue ?? 0)
              ...
              {!prices && !loading && !priceError && (
                <div className="text-center py-8 text-gray-500 text-sm">
                  Market prices will appear here once loaded.
                </div>
              )}
            </div>
          )}
