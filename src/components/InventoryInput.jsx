...
apiEnchantedRawId, apiEnchantedRefinedId, getEnchantedLowerRefinedId,
optimiseMaterial } from '../data/materials'
...
import { saveState, loadState, clearState, formatSavedAt } from '../services/storageService'
import MobileInventoryCard from './MobileInventoryCard'
import ExportModal from './ExportModal'
import RoutePlannerTab from './RoutePlannerTab'

// ─── Helpers ──────────────────────────────────────────────────────────────────
// Cities reachable without red/black zones
...
if (saved.enchantedInventory)             setEnchantedInventory(saved.enchantedInventory)
if (saved.usageFee !== undefined)         setUsageFee(saved.usageFee)
if (saved.useSavedFees !== undefined)     setUseSavedFees(saved.useSavedFees)
if (saved.includeMarketFee !== undefined) setIncludeMarketFee(saved.includeMarketFee)
if (saved.server)                         setServer(saved.server)
if (saved.allowRedBlack !== undefined)    setAllowRedBlack(saved.allowRedBlack)
if (saved.savedAt)                        setSavedAt(saved.savedAt)
}
// Mark load as complete — save effect can now run
...
saveState({ inventory, allocation, enchantedInventory, usageFee, useSavedFees, includeMarketFee, server, allowRedBlack, savedAt: now })
setSavedAt(now)
}, [inventory, allocation, enchantedInventory, usageFee, useSavedFees, includeMarketFee, server])
...
const ids = buildItemIds(activeMats, TIERS)
const priceData = await fetchPrices(ids, server)
setPrices(priceData)
setLastFetched(new Date())

// Compute optimiser data immediately so Route Planner works without visiting Optimiser tab
const allowedCities = allowRedBlack ? null : SAFE_CITIES
const rawInvFor = mat => Object.fromEntries(TIERS.map(t => [t, parseInt(inventory[mat.id]?.[t]) || 0]))
const optimised = MATERIAL_TYPES.map(mat => {
const rawInv = rawInvFor(mat)
if (!TIERS.some(t => rawInv[t] > 0)) return null
const effectiveFee = getEffectiveFee(mat.id)
const base  = optimiseMaterial(mat, rawInv, priceData, BASE_RETURN_RATE,  effectiveFee, includeMarketFee, allowedCities)
const bonus = optimiseMaterial(mat, rawInv, priceData, CITY_RETURN_RATE, effectiveFee, includeMarketFee, allowedCities)
return { mat, base, bonus }
}).filter(Boolean)
setoptimisedData(optimised)
...
<div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-700">
  <button onClick={() => setAllowRedBlack(prev => !prev)}
    className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-medium transition-colors ${
      allowRedBlack
        ? 'bg-red-500/20 border-red-500/40 text-red-400'
        : 'bg-gray-800 border-gray-600 text-gray-400'
    }`}>
    {allowRedBlack ? '⚔️ Red/Black zones enabled' : '🛡️ Blue/Yellow only (safe)'}
  </button>
  <span className="text-xs text-gray-500">
    {allowRedBlack
      ? 'Caerleon and faster routes included — full loot risk'
      : 'Safe routes only — Caerleon excluded'}
  </span>
</div>
...
{/* Mobile View - cards */}
<MobileInventoryCard inventory={inventory} onChange={handleInventoryChange} />
...
<div className="flex items-center justify-between bg-gray-900 border border-gray-700 rounded-xl px-5 py-3">
  <div className="flex items-center gap-4 text-xs text-gray-400">
    {/* Existing refresh/countdown */}
    <span>Prices: <span className="text-gray-200 font-mono">{lastFetched ? lastFetched.toLocaleTimeString() : '—'}</span></span>
    {countdown && (
  ...
  <div className="flex gap-2">
    <button onClick={() => setShowExport(true)}
      className="flex items-center gap-2 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-gray-300 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors">
      🖨️ Export CSV
    </button>
...
allowedCities={allowRedBlack ? null : SAFE_CITIES}
onOptimised={setoptimisedData}
/>
)}
...
prices={prices}
optimisedData={optimisedData}
allowRedBlack={allowRedBlack}
/>
)}
...
{/* Export modal — outside tabs so it works from any tab */}
{showExport && (
<ExportModal
