...
function bestPrice(prices, apiId, allowedCities = null) {
const cityData = prices?.[apiId]
if (!cityData) return null
return Object.entries(cityData)
.filter(([city]) => !allowedCities || allowedCities.includes(city))
...
export function scoreCascade(materialType, rawInventory, rawSplits, prices, returnRate, usageFee = 0, includeMarketFee = true, allowedCities = null) {
...
const bestRawPrice     = bestPrice(prices, row.rawApiId,     allowedCities)
const bestRefinedPrice = bestPrice(prices, row.refinedApiId, allowedCities)
...
export function optimiseMaterial(materialType, rawInventory, prices, returnRate, usageFee = 0, includeMarketFee = true, allowedCities = null) {
...
materialType, rawInventory, forcedSplits, prices, returnRate, 0, includeMarketFee, allowedCities
)
return {
totalSilver,
splits: forcedSplits,
...
const { totalSilver, tierBreakdown } = scoreCascade(
materialType, rawInventory, currentSplits, prices, returnRate, usageFee, includeMarketFee, allowedCities
)
